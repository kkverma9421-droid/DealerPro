...
function ImagePreview({ files, onRemove }: ImagePreviewProps) {
...
function StepBar({ current }: { current: number }) {
  const steps = ['Property Info', 'Details & Media', 'Review']
  return (
        ...
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-6 md:p-8">
          ...
          {step === 0 && (
            <div className="space-y-5">
              ...
              <Field label="Property Title" required>
                <input className={inputCls} placeholder="e.g. 3BHK Premium Apartment"
              ...
              <Field label="Price (₹)" required hint="Enter full amount e.g. 5500000 for ₹55 L">
                <div className="relative">
                  ...
                  <input type="number" className={`${inputCls} pl-7`} placeholder="5500000"
                    value={form.price} onChange={set('price')} min={0} />
                </div>
                {form.price && !isNaN(Number(form.price)) && (
                  <p className="text-xs text-emerald-600 font-semibold mt-1">{formatPrice(form.price)}</p>
                )}
              </Field>
            ...
            <div className="space-y-5">
              ...
              <Field label="Description">
                <textarea
                  ...
                  rows={4}
                  placeholder="Describe the property — layout, amenities, highlights…"
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Latitude" hint="Optional — for map pin">
                  <input type="number" step="any" className={inputCls} placeholder="30.9010"
                    value={form.latitude} onChange={set('latitude')} />
                </Field>
                <Field label="Longitude" hint="Optional — for map pin">
                  <input type="number" step="any" className={inputCls} placeholder="75.8573"
              ...
              <Field label="Property Images" hint="First image becomes primary. Max 8 images.">
                <div
                  className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center cursor-pointer hover:border-emerald-400 hover:bg-emerald-50 transition-all"
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              {files.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">Images</p>
                  <div className="flex flex-wrap gap-2">
                    {files.map((f, i) => (
                      <img key={i} src={URL.createObjectURL(f)} alt=""
                        className={`w-16 h-16 object-cover rounded-xl border-2 ${i === 0 ? 'border-emerald-400' : 'border-slate-200'}`} />
                    ))}
                  </div>
                </div>
              )}
