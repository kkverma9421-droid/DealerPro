    ...
    <div className="flex items-center mb-8">
      {steps.map((s, i) => (
        <div key={s} className="flex items-center flex-1 last:flex-none">
          ...
          {i < steps.length - 1 && (
            <div className={`flex-1 h-px mx-2 sm:mx-3 ${i < current ? 'bg-emerald-400' : 'bg-slate-200'}`} />
          )}
        </div>
      ))}
    </div>
...
// ─── Wizard ───────────────────────────────────────────────────────────────────
export default function PropertyFormWizard({ onSuccess, onCancel }: PropertyFormWizardProps) {
  const [step,          setStep]          = useState(0)
  const [files,         setFiles]         = useState<File[]>([])
  const [loading,       setLoading]       = useState(false)
  const [loadingMsg,    setLoadingMsg]    = useState('Saving property…')
        ...
        {step === 0 && (
          <div className="space-y-5">
            ...
            <Field label="Property Title" required>
              <input
                className={inputCls}
                placeholder="e.g. 500 Sq Yd Plot near Prem Mandir, Vrindavan"
                value={form.title}
                onChange={set('title')}
              />
            </Field>
          ...
          <div className="space-y-5">
            ...
            <Field label="Description">
              <textarea
                ...
                rows={4}
                placeholder="Describe the property — layout, amenities, nearby landmarks…"
            ...
            <Field label="Property Images" hint="First image becomes primary. Max 8 files.">
              <div
          ...
          <div className="space-y-5">
            ...
            <div className="bg-slate-50 rounded-xl p-5 space-y-3">
              {([
                ...
                ['Area',     form.area ? `${form.area} sq yd` : '—'],
                ['Bedrooms', form.bedrooms || '—'],
                ['Featured', form.featured ? '⭐ Yes' : 'No'],
                ['Images',   `${files.length} selected`],
              ] as [string, string][]).map(([label, value]) => (
                <div key={label} className="flex justify-between items-start gap-4">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wide w-24 shrink-0">
                    {label}
                  </span>
